...
# === 参数 ===
timesteps = 15
variables = ["u10", "v10", "sst", "t2m", "msl", "tp"]  # 输入中加入降水
target_var = "tp"  # 预测降水
target_idx = variables.index(target_var)
data_path = "merged_1.5deg_no_norm_with_tp.nc"
batch_size = 4
epochs = 250
...
H, W = ds[variables[0]].shape[1:]  # height, width
data_stack = np.stack([ds[var].values for var in variables], axis=1)  # (T, 6, H, W)
...
class ClimateDataset(Dataset):
    def __init__(self, data, timesteps, start_idx=0, end_idx=None):
        ...
        self.end_idx = end_idx or (len(data) - timesteps - 17 + 1)
    ...
    def __getitem__(self, idx):
        idx += self.start_idx
        x = self.data[idx : idx + self.timesteps]  # (timesteps, C, H, W)
        y = self.data[idx + 16, target_idx]       # 第17个月为预测目标
        return torch.tensor(x, dtype=torch.float32), torch.tensor(y, dtype=torch.float32)

# === 数据划分 ===
total_samples = data_stack.shape[0] - timesteps - 17 + 1
...
class ClimateTCNModel(nn.Module):
    ...
    def forward(self, x):  # x: (B, T, C, H, W)
        ...
        x = x.view(B * T, C, H, W)
        x = self.encoder(x)
        x = x.view(B, T, -1).transpose(1, 2)  # (B, embed_dim, T)
        x = self.tcn(x)
        x = x[:, :, -1]
        out = self.fc(x)
...
for epoch in range(start_epoch, epochs):
    ...
    if epoch_loss < best_loss:
        ...
        torch.save(model.state_dict(), "climate_tcn_with_timeencoding_150_1month_tp.pth")
        print(f"🌟 当前为最优模型，保存为 climate_tcn_with_timeencoding_150_1month_tp.pth（Loss: {best_loss:.4f}）")
